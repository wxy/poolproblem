...
@Test func hardLinksShareInodeAndDedup() throws {
    ...
    let records = [
        ...
        FileRecord(itemID: "a", url: dst, allocatedBytes: 1_000_000, deviceID: dstStat.st_dev, inode: dstStat.st_ino, lastModified: .distantPast),
    ]
...
`statFile` 辅助函数位于测试文件末尾；硬链接共享 inode，去重成立。
...
同时修改 `Scanner.scan`：`items` 在返回前经过 `ReclaimableEstimator().apply(to:records:)`，使 `reclaimableBytes` 为硬链接去重后数值。

**已证实的平台事实（2026-08-09 实测，macOS 26 / APFS）**：

- `clonefile`/`cp -c` 产生的克隆文件 inode 不同、`fileResourceIdentifier` 不同，但物理块共享（实测删除 100MB 克隆仅释放 16KB）；
- 因此删除前无法通过公开 per-file API 精确测量"真实可释放"，`allocatedBytes` 对克隆密集型目录（XCTestDevices、模拟器数据）是上界；
- 诚实计量落地：扫描输出标注克隆风险（表观大小可能虚高），真实释放量在清理后用量规实测（Task 13 的 `actualFreedBytes`）。
...
public struct Cleaner: Sendable {
    public init(
        ...
        availableBytesReader: @escaping @Sendable (URL) -> Int64 = { VolumeReader.read(fileURL: $0).availableBytes },
        now: @escaping () -> Date = Date.init
    )
...
说明：`CleanOutcome.freedBytes` 是删除器估算之和（上界），`actualFreedBytes` 是量规实测（清理前后 `availableBytes` 差值），二者都输出，如实反映 APFS 克隆的"表观 vs 真实"。

`ScanResult` 增加 `volumeURL: URL`（`Scanner.scan` 填充为 `homeDirectory` 对应的卷 URL），供 `Cleaner` 实测前后量。
...
- **诚实计量修订（2026-08-09 实测驱动）**：APFS 克隆文件 inode 与资源标识均不共享但物理块共享，公开 API 无法删除前精确测量；Task 6 去重仅覆盖硬链接（inode 相同），克隆风险在扫描输出标注，真实释放由 Task 13 的 `actualFreedBytes`（量规实测）兜底。
